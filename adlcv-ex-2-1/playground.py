...
if __name__ == '__main__':
    ...
    diffusion = Diffusion(T=500, beta_start=1e-4, beta_end=0.02, device=device, schedule='linear')
    plt.figure()
    plt.plot(range(1,diffusion.T+1), diffusion.alphas.cpu().numpy(), label='alphas lin', linewidth=3)
    plt.plot(range(1,diffusion.T+1), diffusion.alphas_bar.cpu().numpy(), label='alphas_bar lin',linewidth=3)
    plt.plot(range(1,diffusion.T+1), diffusion.betas.cpu().numpy(), label='betas lin', linewidth=3)
    diffusion = Diffusion(T=500, beta_start=1e-4, beta_end=0.02, device=device, schedule='cosine')
    plt.plot(range(1,diffusion.T+1), diffusion.alphas.cpu().numpy(), label='alphas cos', linestyle='--')
    plt.plot(range(1,diffusion.T+1), diffusion.alphas_bar.cpu().numpy(), label='alphas_bar cos', linestyle='--')
    plt.plot(range(1,diffusion.T+1), diffusion.betas.cpu().numpy(), label='betas cos', linestyle='--')
