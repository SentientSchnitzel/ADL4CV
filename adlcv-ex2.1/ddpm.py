...
class Diffusion:
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        # the mean of the distribution p(x_{t-1} | x_t)
        mean = 1 / torch.sqrt(alpha) * (x_t - beta/(torch.sqrt(1-alpha_bar)) * predicted_noise)  # Eq. 11 in the ddpm paper 
        std = torch.sqrt(beta)
    ...
    def p_sample(self, model, x_t, t):
        ...
        noise = torch.where(t.view(-1, 1, 1, 1) > 1, torch.randn_like(x_t), torch.zeros_like(x_t))
