...
class ViT(nn.Module):
    ...
    def forward(self, img):
        # The input image has shape (batch_size, channels, H, W)
        # Transform the image to patches and project to the embedding dimension
        tokens = self.to_patch_embedding(img)
        batch_size, num_patches, embed_dim = tokens.size()
        
        if self.pool == 'cls': # cls is the first token
